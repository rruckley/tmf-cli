...
use super::{display_desc, display_name, display_opt, iterate_name, TMFOperation};
...
pub fn handle_tmf633(
    ...
    match module {
        TMF633Modules::Catalog { op } => match op {
        ...
        TMF633Modules::Category { op } => match op {
            TMFOperation::Create { name, desc } => {
                let category = ServiceCategory::new(name).description(desc.unwrap_or_default());
        ...
        TMF633Modules::Specification { op } => match op {
            TMFOperation::Create { name, desc } => {
                let specification =
                    ServiceSpecification::new(name).description(desc.unwrap_or_default());
